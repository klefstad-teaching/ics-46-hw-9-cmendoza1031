...
vector<int> extract_shortest_path(const vector<int>& distances, const vector<int>& previous, int destination) {
    ...
    if ((previous[destination] == -1 && destination != 0) || distances[destination] == INF)
        return path;
    ...
    for (int at = destination; at != -1; at = previous[at]) {
        ...
        // break once we reach the source to avoid potentinal infinite loops
        if (at == 0) break; // NEW
    }
